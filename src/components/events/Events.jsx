  ...
  const fetchData = async () => {
    try {
      const res = pageNumber
        ? await axios.get(
            `${process.env.REACT_APP_BASE_BACKEND_URL}/event/events?page=${pageNumber}`
          )
        : await axios.get(
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await axios.delete(
        `${process.env.REACT_APP_BASE_BACKEND_URL}/event/delete?_id=${id}`
      );
    ...
    const filterEvents = async () => {
      try {
        const res = await axios.get(
          `${process.env.REACT_APP_BASE_BACKEND_URL}/event/events?q=${input}`
        );
        console.log("Hello    fetchData   res:", res);
        setEvents(res.data);
      } catch (error) {
