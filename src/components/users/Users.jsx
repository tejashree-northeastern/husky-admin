    ...
    try {
      const res =
        input && input.length >= 3
          ? await axios.get(
              `${process.env.REACT_APP_BASE_BACKEND_URL}/user/getAll?q=${input}`
            )
          : await axios.get(
              `${process.env.REACT_APP_BASE_BACKEND_URL}/user/getAll`
            );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await axios.delete(
        `${process.env.REACT_APP_BASE_BACKEND_URL}/user/delete/${id}`
      );
